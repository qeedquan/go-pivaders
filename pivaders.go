...
var (
	Res         = f64.Vec2{800, 600}
	BulletSize  = f64.Vec2{5, 10}
	MissileSize = f64.Vec2{5, 5}
	BlockSize   = f64.Vec2{10, 10}
	AlienSize   = f64.Vec2{30, 40}
)
	...
	f := func(a *Ammo, v reflect.Value) {
		...
		if collide(&a.Entity, e) {
			...
			if typ == 'a' {
				...
				alien.ExpPos = f64.Vec2{alien.X, alien.Y}
			} else if typ == 'p' {
				playSound(sfx.explosion, 1)
			}
...
func newImageUniform(color color.Color, size f64.Vec2) *Image {
...
func newAmmo(color color.Color, size f64.Vec2) *Ammo {
	return &Ammo{
		Entity: Entity{
...
func newBarrier(color color.Color, col, row, spacing int) *Barrier {
	...
	return &Barrier{
		Entity: Entity{
			...
			Vec2:  f64.Vec2{float64(x), float64(y)},
			Size:  BlockSize,
		},
	}
}
